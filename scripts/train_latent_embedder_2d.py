...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train autoencoder (latent embedder)")
    parser.add_argument(
        "--btxrd-path",
        required=True,
        help="Relative path to BTXRD dataset",
    )
    parser.add_argument(
        "--data-split-path",
        required=True,
        help="Relative path to data-split.json",
    )
    args = parser.parse_args()

    # --------------- Settings --------------------
    current_time = datetime.now().strftime("%Y_%m_%d_%H%M%S")
    path_run_dir = Path("runs") / f"vae_{str(current_time)}"
