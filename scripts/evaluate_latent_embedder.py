...
path_out = Path("results") / "BTXRD" / "metrics"
...
# -------------- Helpers ---------------------
pil2torch = (
    lambda x: torch.as_tensor(np.array(x)).moveaxis(-1, 0)
)  # In contrast to ToTensor(), this will not cast 0-255 to 0-1 and destroy uint8 (required later)

# ---------------- Dataset/Dataloader ----------------
# ds_real = ImageFolder(
#     "/mnt/hdd/datasets/pathology/kather_msi_mss_2/train/", transform=pil2torch
# )
...
# )
dm_real = dm.test_dataloader()

logger.info(f"Samples Real: {len(ds_real)}")


# --------------- Load Model ------------------
model = VAE.load_from_checkpoint("./runs/vae_2025_11_25_170041/last.ckpt")
model.to(device)
